...
float movc = 0;//desplazamiento horizpntal 
...
void dibujarNueveCirculos(int numColumnas, int numFilas, float zonaX, float anchoCelda, float altoCelda, float desplazamientox, float tamPequenoCirculo) {
  
  for (int col = 0; col < numColumnas; col++) {
    for (int fila = 0; fila < numFilas; fila++) {
      //posicion 
      float posiX = zonaX + col * anchoCelda + anchoCelda / 2;
      float posiY = fila * altoCelda + altoCelda / 2;
      float x = posiX + desplazamientox;
      float y = posiY;
